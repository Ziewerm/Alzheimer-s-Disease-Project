...
```{r See what version each package is}
pkgs <- c(
  "openxlsx", "tidyverse", "dplyr", "naniar", "ggplot2", "patchwork",
  "readr", "heatmaply", "gtsummary"
)
...
```{r load and filter}
Lunbeck_plasma <- read.xlsx("path to file")
...
table1 <-
  Lunbeck_plasma |> 
  tbl_summary(include = c(AgeAtVisit,Gender, Cerebrospinal.fluids_Amyloid.beta.42.pg.ml, Cerebrospinal.fluids_Phospho.Tau.pg.ml, Cerebrospinal.fluids_Total.Tau.pg.ml), by = Class)
...
```{r remove columns and rows with na values}
...
for (i in 1:7) {
  ...
  file_path <- paste0("path to file", i, ".csv")
...
if (length(benchmark_vars) >= 2) {
  ...
  file_path_benchmark <- "Path to file"
